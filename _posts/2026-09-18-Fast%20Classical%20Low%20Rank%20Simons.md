...
Given is an implementation of a fast (polylog time) classical solution to Simon's problem with respect to Simon's problems posed for functions representable in low tensor (CP) rank. The main thrust of it is to just keep the tensor factorization as you go along.
